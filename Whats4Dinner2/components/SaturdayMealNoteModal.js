...
const SaturdayMealNoteModal = ({visible, whatDay, onClose, onSubmit}) => {
    ...
    let SATURDAY_TITLE_KEY = '@Saturday_Title_input';
    let SATURDAY_DESC_KEY = '@Saturday_Desc_input';
...
// Returns: None
    const onSubmitData = async () => {
        try {
            await AsyncStorage.setItem(SATURDAY_TITLE_KEY, MealTitle)
            await AsyncStorage.setItem(SATURDAY_DESC_KEY, MealDescription)
        } catch (err) {
    ...
    const getData = async () => {
        try{
            const curTitle = await AsyncStorage.getItem(SATURDAY_TITLE_KEY)
            const curDesc = await AsyncStorage.getItem(SATURDAY_DESC_KEY)
            if (curTitle !== null) {
                setTitle(curTitle)
    ...
    const _clearTitle = async () => {
        try {
            await AsyncStorage.removeItem(SATURDAY_TITLE_KEY)
        } catch {
    ...
    const _clearDesc = async () => {
        try {
            await AsyncStorage.removeItem(SATURDAY_DESC_KEY)
        } catch {
