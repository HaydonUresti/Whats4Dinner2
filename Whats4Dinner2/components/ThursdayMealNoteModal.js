...
const ThursdayMealNoteModal = ({visible, whatDay, onClose, onSubmit}) => {
    ...
    let THURSDAY_TITLE_KEY = '@Thursday_Title_input';
    let THURSDAY_DESC_KEY = '@Thursday_Desc_input';
...
// Returns: None
    const onSubmitData = async () => {
        try {
            await AsyncStorage.setItem(THURSDAY_TITLE_KEY, MealTitle)
            await AsyncStorage.setItem(THURSDAY_DESC_KEY, MealDescription)
        } catch (err) {
    ...
    const getData = async () => {
        try{
            const curTitle = await AsyncStorage.getItem(THURSDAY_TITLE_KEY)
            const curDesc = await AsyncStorage.getItem(THURSDAY_DESC_KEY)
            if (curTitle !== null) {
                setTitle(curTitle)
    ...
    const _clearTitle = async () => {
        try {
            await AsyncStorage.removeItem(THURSDAY_TITLE_KEY)
        } catch {
    ...
    const _clearDesc = async () => {
        try {
            await AsyncStorage.removeItem(THURSDAY_DESC_KEY)
        } catch {
...
export default ThursdayMealNoteModal;