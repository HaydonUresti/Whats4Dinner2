...
const SundayMealNoteModal = ({visible, whatDay, onClose, onSubmit}) => {
    ...
    let SUNDAY_TITLE_KEY = '@Sunday_Title_input';
    let SUNDAY_DESC_KEY = '@Sunday_Desc_input';
...
// Returns: None
    const onSubmitData = async () => {
        try {
            await AsyncStorage.setItem(SUNDAY_TITLE_KEY, MealTitle)
            await AsyncStorage.setItem(SUNDAY_DESC_KEY, MealDescription)
        } catch (err) {
    ...
    const getData = async () => {
        try{
            const curTitle = await AsyncStorage.getItem(SUNDAY_TITLE_KEY)
            const curDesc = await AsyncStorage.getItem(SUNDAY_DESC_KEY)
            if (curTitle !== null) {
                setTitle(curTitle)
    ...
    const _clearTitle = async () => {
        try {
            await AsyncStorage.removeItem(SUNDAY_TITLE_KEY)
        } catch {
    ...
    const _clearDesc = async () => {
        try {
            await AsyncStorage.removeItem(SUNDAY_DESC_KEY)
        } catch {
...
export default SundayMealNoteModal;